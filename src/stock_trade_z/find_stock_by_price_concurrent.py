...
def load_stock_data_concurrent(
    data_dir: Path, max_workers: Optional[int] = None
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_file = {
            executor.submit(load_single_stock_data, csv_file): csv_file
...
def find_by_price_single_stock(
    ...
    # 查找所有符合条件的价格
    mask = (df_filtered[price_type] >= min_price) & (
        df_filtered[price_type] <= max_price
    )
...
def find_by_price_concurrent(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_stock = {
            executor.submit(search_func, stock_item): stock_item[0]
...
def main():
    ...
    parser.add_argument(
        "--tolerance", type=float, default=0.01, help="价格容差 (默认: 0.01)"
    )
